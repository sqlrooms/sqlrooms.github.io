...
| Type Parameter | Description |
| ------ | ------ |
| `Schema` *extends* `ZodType`<`any`, `ZodTypeDef`, `any`> | The Zod schema type that defines the shape and validation of each row |
...
| Name | Type |
| ------ | ------ |
|  `data` | `undefined` | [`UseSqlQueryResult`](../interfaces/UseSqlQueryResult.md)<`TypeOf`<`Schema`>> |
|  `error` | `null` | `Error` |
|  `isLoading` | `boolean` |
