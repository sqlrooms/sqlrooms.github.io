---
url: 'https://sqlrooms.org/api/duckdb/type-aliases/DuckDbSliceState.md'
---
...
| `executeSql()` | (`query`, `version?`) => `Promise`<[`QueryHandle`](QueryHandle.md) | `null`> | Execute a query with query handle (not result) caching and deduplication |
...
| `dropRelation()` | (`tableName`) => `Promise`<`void`> | Delete a table or view with optional schema and database. |
