---
url: /api/ai/functions/cleanupPendingAnalysisResults.md
---
[@sqlrooms/ai](../index.md) / cleanupPendingAnalysisResults
...
| ------ | ------ | ------ |
| `session` | { `id`: `string`; `name`: `string`; `modelProvider`: `string`; `model`: `string`; `customModelName?`: `string`; `baseUrl?`: `string`; `analysisResults`: `object`\[]; `createdAt?`: `Date`; `uiMessages`: `object`\[]; `toolAdditionalData?`: `Record`<`string`, `unknown`>; `messagesRevision`: `number`; `prompt`: `string`; `isRunning`: `boolean`; `lastOpenedAt?`: `number`; } | The session to clean up |
| `session.id` | `string` | - |
| `session.name` | `string` | - |
