...
|  `props` | `readonly` | `Readonly`<`P`> | - | `Component.props` |
|  `state` | `public` | `Readonly`<`S`> | - | `Component.state` |
...
| Parameter | Type |
| ------ | ------ |
| `state` | `IState` | (`prevState`, `props`) => `IState` | `Pick`<`IState`, `K`> | `null` | `Pick`<`IState`, `K`> | `null` |
| `callback?` | () => `void` |
...
| Parameter | Type |
| ------ | ------ |
| `option` | `boolean` | `SVGProps`<`SVGTextElement`> | `ReactElement`<`SVGElement`, `string` | `JSXElementConstructor`<`any`>> | (`props`) => `ReactElement`<`SVGElement`> | `undefined` |
| `props` | `any` |
| `value` | `ReactNode` |
