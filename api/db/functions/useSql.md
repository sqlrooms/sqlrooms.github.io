---
url: 'https://sqlrooms.org/api/db/functions/useSql.md'
---
[@sqlrooms/db](../index.md) / useSql
...
| ------ | ------ | ------ |
| `options` | { `query`: `string`; `enabled?`: `boolean`; `version?`: `number`; } | Configuration object containing the query and execution control |
...
| Name | Type |
| ------ | ------ |
| `data` | `UseSqlQueryResult`<`output`<`Schema`>> | `undefined` |
| `error` | `Error` | `null` |
| `isLoading` | `boolean` |
