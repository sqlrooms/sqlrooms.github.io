---
url: 'https://sqlrooms.org/api/ai-settings/functions/createAiSettingsSlice.md'
---
[@sqlrooms/ai-settings](../index.md) / createAiSettingsSlice

# Function: createAiSettingsSlice()

> **createAiSettingsSlice**(`props?`): `StateCreator`<[`AiSettingsSliceState`](../type-aliases/AiSettingsSliceState.md)>

## Parameters

| Parameter | Type |
| ------ | ------ |
| `props?` | `CreateAiSettingsSliceParams` |

## Returns

`StateCreator`<[`AiSettingsSliceState`](../type-aliases/AiSettingsSliceState.md)>
